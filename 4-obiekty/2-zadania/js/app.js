...
function fixData(data) {
    return data.map(([id, name, surname, email, age, cash]) => ({id, name, surname, email, age, cash}));
}
...
const totalMoney = fixedUsers.reduce((sum, user) => sum + user.cash, 0);
console.log("Łączna suma pieniędzy wszystkich użytkowników:", totalMoney);
const totalAge = fixedUsers.reduce((sum, user) => sum + user.age, 0);
const averageAge = totalAge / fixedUsers.length;
console.log("Średni wiek użytkowników:", averageAge);
const emails = fixedUsers.map(user => user.email);
console.log("Maile użytkowników:", emails);
